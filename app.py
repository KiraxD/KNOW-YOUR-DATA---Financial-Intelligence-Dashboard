"""
ERP Financial Intelligence Dashboard - Flask Backend
...
def clean_amount(val):
    ...
    val_str = re.sub(r'\b(USD|INR|AED|EUR|GBP|JPY|CAD|AUD|CHF|CNY|SGD|HKD|NOK|SEK|DKK|THB|MYR|IDR|PHP|KRW)\b', '', val_str, flags=re.IGNORECASE)
...
def classify_transaction(vendor, description, category_hint=None):
    ...
    for rule in CLASSIFICATION_RULES:
        ...
        if matched:
            raw_score = min(1.0, (len(matched) / max(len(rule['keywords']) * 0.3, 1)) * rule['weight'])
...
def generate_insights(df, kpis, col_map):
    ...
    if '_category' in df.columns:
        ...
        saas_cloud = cat_counts.get('SaaS Subscriptions', 0) + cat_counts.get('Cloud Infrastructure', 0)
        if saas_cloud > 0:
    ...
    if vendor_col and vendor_col in df.columns and '_clean_amount' in df.columns:
        vendor_spend = df.groupby(vendor_col)['_clean_amount'].sum().dropna().sort_values(ascending=False)
        if len(vendor_spend) > 0:
            top_vendor = vendor_spend.index[0]
            top_pct = round(vendor_spend.iloc[0] / vendor_spend.sum() * 100, 1) if vendor_spend.sum() > 0 else 0
            if top_pct > 40:
                insights.append({'type': 'WARNING',
    ...
    if dept_col and dept_col in df.columns and '_clean_amount' in df.columns:
        dept_spend = df.groupby(dept_col)['_clean_amount'].sum().dropna().sort_values(ascending=False)
        if len(dept_spend) > 1:
            top_dept = dept_spend.index[0]
            top_dept_pct = round(dept_spend.iloc[0] / dept_spend.sum() * 100, 1) if dept_spend.sum() > 0 else 0
            insights.append({'type': 'INFO',
    ...
    if not insights:
        insights.append({'type': 'INFO', 'message': 'Dataset processed successfully. No major anomalies detected.'})
...
def process_dataset(filepath):
    ...
    dominant_currency = max(currencies, key=currencies.get) if currencies else 'Unknown'
    ...
    for idx, row in df.iterrows():
        ...
        sev, flags = assess_risk(row, amount_col, vendor_col, desc_col, date_col, is_dup)
    ...
    # ─── KPI Calculations ───
    total = len(df)
    invalid_amounts = int(df['_clean_amount'].isna().sum()) if '_clean_amount' in df.columns else 0
    missing_vendors = int(df[vendor_col].isna().sum() | (df[vendor_col] == '').sum()) if vendor_col else 0
    duplicate_count = len(dup_indices)
    dup_exposure = float(df.loc[list(dup_indices), '_clean_amount'].sum()) if dup_indices and '_clean_amount' in df.columns else 0.0

    personal_mask = df['_category'] == 'Personal Expense'
    personal_exposure = float(df.loc[personal_mask, '_clean_amount'].sum()) if '_clean_amount' in df.columns else 0.0

    saas_cloud_mask = df['_category'].isin(['SaaS Subscriptions', 'Cloud Infrastructure'])
    saas_spend = float(df.loc[saas_cloud_mask, '_clean_amount'].sum()) if '_clean_amount' in df.columns else 0.0
    ...
    total_spend = float(df['_clean_amount'].dropna().sum()) if '_clean_amount' in df.columns else 0.0
    ...
    csv_path = os.path.join(app.config['EXPORTS_FOLDER'], f'cleaned_{export_id}.csv')
    xlsx_path = os.path.join(app.config['EXPORTS_FOLDER'], f'cleaned_{export_id}.xlsx')
    ...
    report_path = os.path.join(app.config['INSIGHTS_FOLDER'], f'report_{export_id}.json')
...
def download_cleaned():
    ...
    if fmt == 'xlsx':
        path = os.path.join(app.config['EXPORTS_FOLDER'], f'cleaned_{export_id}.xlsx')
        mimetype = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    else:
        path = os.path.join(app.config['EXPORTS_FOLDER'], f'cleaned_{export_id}.csv')
        mimetype = 'text/csv'
